...
export async function sendEmail(env, { key, to, subject, heading, body, actionUrl, actionLabel, attachments = [] }) {
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { "Authorization": `Bearer ${env.RESEND_API_KEY}`, "Content-Type": "application/json", "Idempotency-Key": key }, body: JSON.stringify({ from: env.EMAIL_FROM || "RIVAL PRAXIS <orders@rivalpraxis.com>", to: recipients, subject, html, attachments }), signal: AbortSignal.timeout(15000) });
